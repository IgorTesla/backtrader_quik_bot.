...
def SaveCandlesToFile(classCode='TQBR', secCodes=('SBER',), timeFrame='MN', compression=1,
    ...
    for secCode in secCodes:  # Пробегаемся по всем тикерам
        ...
        newBars = qpProvider.GetCandlesFromDataSource(classCode, "RIM3", 1,0)["data"]  # Получаем все свечки
        print(newBars)
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
   ...
   # SaveCandlesToFile(classCode, secCodes, 'M', 15, skipFirstDate=True, skipLastDate=True)  # 15-и минутные бары
    SaveCandlesToFile(classCode, secCode, 'M', 1, skipFirstDate=True, skipLastDate=True)  # 5-и минутные бары
